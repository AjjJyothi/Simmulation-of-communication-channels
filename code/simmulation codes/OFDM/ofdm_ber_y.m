...
% Modulation method: BPSK, OPSK, BPSK, 160AX, 320AX, 6112:1t4 mod mod_method - 'OPSK'; 
...
Mod_methods = ('BPSk','QPSK','8PSK','160AM','320AM','640AM'); mod_order = find(ismember(mod_methods,mod_method)); 

%% Input data to binary stream

 im = imread('baboon.bmpl');
 im_bin = dec2bin(im(:))';
 im_bin = im_bin(:); 
...
% Pads input signal to appropriate length
 ...
 sym_rem = mod(mod_order_mod(length(im_bin),mod_order),mod_order)'; padding = repmat('0',sym_rem,1);
 im_bin_padded = (im_bin;padding);
cons_data = reshape(im_bin_padded,mod_order,lengt(im_bin_padded)/mod_order)';
...
% BPSK
 if mod_order ==1
 	...
 	in_phase = cos(n);
	 quadrature = sin(n);
 	Symbol_book = (in_phase + quadrature*11)'; 
  end 
...
if mod_order == 2 || mod_order == 3
	 mod_ind = 2^(mod_order-1);
	 n=0:pi/mod_ind;2*pi-pi/mod_ind;
	 in_phase = repmet(linspace(-1,1,mod_ind),mod_ind,1); 	 	 	 quadrature = sin(n+pi/4);
...
% 16QAM, 64QAM modulation
 if mod_order == 4 || mod_order == 6
	 ...
	 in_phase = repmet(linspace(-1,1,mod_ind),mod_ind,1); 	 	 quadrature = repmat(linspace(-1,1,mod_ind)',1,mod_ind); 		 symbol_book = in_phase(:) + quadrature(:)*1i;
 end 
...
% Add cyclic prefix extension and shift from parallel to serial  x_cpe = [x(end-n_cpe+1:end, :);x];
 x_s = x_cpe(:); 
...
% Add noise to channel
 ...
 noise = normrnd(0,sqrt(noise_pwr/2),size(x_s)) + normrnd(0,sgrt(noise_pwr/2),size(x_s))*1i;
 x_s_noise = x_s + noise; 
...
% Remove cyclic prefix extension and shift from serial to parallel x_p = reshape(x_s_noise_fading;n_fft+n_cpe,length(x_s_noise_fading)/(n_fft+n_cpe));
...
% Parse to binary stream & remove symbol padding
 ...
 rec_im_bin = reshape(rec_syms_cons',numel(rec_syms_cons),1);  rec_im_bin = rec_im_bin(1:end-sym_rem);
...
% Transmit constellation
 ...
 ylabel('Quadreture');
 if n_taps > 1
	title(sprintf('\\bfreceived Constellation\n\\rnMeasured SNR: %.2f dB\nChannel Estimation: %s',snr_means,ch_est_method));
 else
	 title(sprintf('\\bfReceived Constellation\n\\rnMeasured SNR: %.2f dB',snr_means)); 
 end
 grid on; 
...
%Save figure
 if save_file
	 print(sprintf ('Plots/%s_%.0ffft_%.ofcpe_%0fdB_%.0ftaps_%s', mod_method, n_fft,n_cpe, snr, n_taps,ch_est_method),'-dmeta') ;
 end  
